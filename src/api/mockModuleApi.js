...
class ModuleApi {
  static mapping(module, lang) {
    let currentLang = 'uk';
    if (lang) {
      currentLang = lang;
    }
    const currentModule = Object.assign({}, module);
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        if (id) {
          ...
          resolve(Object.assign({}, this.mapping(currentModule, lang)));
        }
      }, delay);
    });
  }
...
// Should find first module that have not all passed units
  static getLatest(lang) {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve(Object.assign({}, this.mapping(modulesData.modules[0], lang)));
      }, delay);
    });
  }
}
